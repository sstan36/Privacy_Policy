...
- **Two Git check-ins (keep in sync):** Treat **`FirebaseApi/docs/privacy_policy.md`** as the **canonical** copy (commit to the **FirebaseApi** repo). When you change policy text, update **`privacy_policy.md`**, then refresh **`privacy_policy.html`** (same substance; public page uses plain layout and a Singapore **PDPA**-oriented caveat plus cross-border notice—see HTML file) and mirror into the **Privacy_Policy** repo root: **`index.html`**, **`privacy_policy.html`**, **`privacy_policy.md`**. **GitHub Pages** primary URL: `https://sstan36.github.io/Privacy_Policy/privacy_policy.html`. The in-app link is **`SettingsSupportConstants.privacyPolicyURL`** / **`privacyPolicyURLString`** (must use the `.html` page).
